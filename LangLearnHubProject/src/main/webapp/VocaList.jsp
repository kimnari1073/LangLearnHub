...
<head>
    ...
    <style>			    			 	    	
     ...
     <script>
    	...
    	const pageList = 15; // 한개의 페이지에 보여질 목록 개수
    	const pageMax = 1; // 최대 생성될 페이지 개수 (페이지를 더보려면 이전, 다음 버튼 클릭해야함)    	
    	...
    	function tableInsert(){
    		...
    		<%	int i =0;
				for(Entry<String, Integer> entry: vocaList.entrySet()) {
					%>var jsonObject = {"idx": "<%= ++i %>", "title":"<%= entry.getKey() %>", "count": "<%= entry.getValue()%>"}
